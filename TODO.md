...
- [x] GMF Mode A single-weapon targeting — pulled out of the uniform buff total; auto-targets the "best" natural weapon (`gmfBestWeapon()`: most attacks of one weapon name, tiebreak highest bonus) and applies per-row only to that weapon. Lion → claws (2 atks) not bite; Zerda → both bites (one weapon). Chosen rows show a ✦ marker. Enhancement non-stacks with `gmfAll` (best weapon = max). Weapon grouped by name (both claws count together — common reading, not strict one-claw RAW). Not yet applied to rake/auto-damage rows (future).
